-- Clear highlights on search when pressing <Esc> in normal mode (but not in terminal)
--  See `:help hlsearch`
vim.api.nvim_create_autocmd("BufEnter", {
	pattern = "*",
	callback = function()
		if vim.bo.buftype ~= "terminal" then
			vim.keymap.set("n", "<Esc>", "<cmd>nohlsearch<CR>", { desc = "clear search highlight", buffer = true })
		end
	end,
})
...
vim.keymap.set(
	"n",
	"<leader>=",
	':lua require("compter").increase()<CR>',
	{ desc = "+ number", remap = true, silent = true }
) -- increment
vim.keymap.set(
	"n",
	"<leader>-",
	':lua require("compter").decrease()<CR>',
	{ desc = "- number", remap = true, silent = true }
) -- decrement
-- Diagnostic keymaps
...
vim.api.nvim_set_keymap(
	"n",
	"<leader>db",
	":lua Close_snacks_dashboard_and_toggle_dbui()<CR>",
	{ desc = "Toggle [DB]", noremap = true, silent = true }
)
...
vim.api.nvim_set_keymap(
	"n",
	"<leader>vd",
	"<cmd>lua Open_in_visidata()<CR>",
	{ desc = "[V]iew [D]ata", noremap = true, silent = true }
)
